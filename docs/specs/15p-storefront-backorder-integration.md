...
- Make 15P purchasable on backorder with regular price **$379.00** and sale price **$299.00**.
...
| R3 | 15P regular `$379`, sale `$299` | **Not true** | Local WooCommerce product currently has empty regular, sale, and effective prices. |
...
- **R4 — Pricing:** 15P must expose `$379.00` as its regular price and `$299.00` as its current sale/effective price.
...
- **R19 — SEO structured data:** Homepage, Shop, Compare, and PDP JSON-LD must be parseable, non-duplicative, URL-consistent, and accurately represent the 15P `$299` BackOrder Offer without unverified delivery timing.
...
- **AC5:** WooCommerce reports 15P regular price `379`, sale/effective price `299`, purchasing enabled, and backorders allowed after both direct implementation and a full fixture refresh.
- **AC6:** A guest can open the 15P PDP, see `$379.00` struck through and `$299.00` active, add one unit to cart, and reach checkout with a clear backorder notice and no invented fulfillment date.
...
- **AC20:** Exactly one 15P Product node exists on PDP with a `$299.00` Offer and `https://schema.org/BackOrder`; no 15P Offer contains an unverified handling/transit estimate.
...
- No price, commerce-state, deployment, or scope decision remains open: the user approved `$379/$299`, backorders enabled now, local-main commit, and no deployment.
