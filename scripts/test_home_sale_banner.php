...
foreach (['en_US' => '', 'zh_TW' => '/zh', 'zh_CN' => '/zh-cn'] as $locale => $prefix) {
    ...
    $p = new WC_Product();
    $html = render($p);
    if (!str_contains($html, '$239.00') || str_contains($html, '$34.01') || !str_contains($html, 'href="' . $prefix . '/product/')) { throw new RuntimeException('Localized price/link failed'); }
